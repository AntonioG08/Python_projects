...
while se_ejecuta:
    ...
    for e in range(cantidad_enemigos):
        ...
        if enemigo_y[e] > 400:
            ...
            texto_final()
            break
        enemigo_x[e] += enemigo_cambio_horizontal[e]

    # Condicional para mantener en los bordes al ENEMIGO
        if enemigo_x[e] <= 0:
            enemigo_cambio_horizontal[e] = 2
            enemigo_y[e] += enemigo_cambio_vertical[e]
        elif enemigo_x[e] >= 736:
            enemigo_cambio_horizontal[e] = -2
            enemigo_y[e] += enemigo_cambio_vertical[e]
    # Colisión
        existe_colision = colision(enemigo_x[e], enemigo_y[e], bala_x, bala_y)
        if existe_colision:
